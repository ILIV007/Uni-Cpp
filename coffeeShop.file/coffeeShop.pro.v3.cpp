...
void store :: mojodi()///mavad avalee khonde shan
{
   ...
   cout<<"mavad avalee be tartieb file vared shavad! (777 to exite)\n\n";
            ...
            string name;
            while(getline(store_filei,name))
            {
                cout<<name<<endl;
            }
            store_filei.close();
    ofstream store_fileo("coffeeShop.store.txt",ios::app);  
    cout<<"num made bar asas file darkhsti(1=>18) \n";
     for(int i=0;i<18;i++)
     {
        cout<<i+1<<"? ";
        cin>>made[i];
         if(made[i]==777)
           break;
        store_fileo<<i+1<<name_made<<" :"<<made[i];
     }
    ...
    coffee_menu[19]="pasta";
     paymenu[19]=210;
         for(int i=0,j=1 ; i<20,j<21 ; i++,j++)
          cout<<j<<'.'<<coffee_menu[i]<<' '<<paymenu[i]<<endl;
   ofstream store_file("coffeeShop.menu.txt");
   for(int i=0,j=1 ; i<20,j<21 ; i++,j++)
    store_file<<j<<'.'<<coffee_menu[i]<<' '<<paymenu[i]<<endl;
   store_file.close();
  }
}
...
void food :: serve_food(store & t)
{
   ...
   for(;;)
   {
   cout<<"\nkodom food nosh az 11=>20? (0 to exit)\n";
   cin>>foodd;
   switch (foodd)
   {
   case 11:
